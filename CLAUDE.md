...
# DB sync (run in this order before commits)
npm run sync-local         # Railway → local predictions.db  (pull new Railway rows)
npm run push-railway       # local DB → Railway              (push new local rows)
npm run sync-railway       # Railway → predictions-export.json (for git commit safety only)
...
IMPORT_SECRET=mlb2026import  # required to push rows to Railway /api/import
```

### Architecture Overview

**`server.js`** — Single-file Express server (~2200 lines). Key flows:

- **Image analysis:** `POST /api/analyze` → multer upload → Claude vision (`SYSTEM_PROMPT`) → `verifyExtraction()` → up to `MAX_VERIFY_PASSES = 3` correction loops → structured game JSON
- **Text/JSON parse:** `POST /api/parse-text` → Claude text extraction via same `SYSTEM_PROMPT` → same structured JSON
- **Prediction:** `POST /api/predict` → auto-computes rolling 5-day OVER rate + 7-day avg total from local DB → injects as context alongside game data → Claude applies v3.16 framework (`PREDICT_SYSTEM`) → `verifyPrediction()` loop → prediction JSON
- **All Claude calls** go through `callClaude()` — a retry wrapper that handles 529 overload errors with exponential backoff (3 attempts, 3s / 6s delays)
- **Persistence:** `POST /api/save-prediction`, `POST /api/result/:id` (grades ML/OU), `GET /api/predictions` (paginated)
- **Manual entry:** `POST /api/manual-entry` — saves a prediction from a raw export string or JSON without running Claude (for back-filling)
- **Flag stats:** `GET /api/flag-stats` — returns live ML/OU accuracy broken down per rule code; used by history page panel (auto-refreshes every 30 s)
- **Sync:** `GET /api/export-all` — full DB dump; always write via Node.js (`JSON.stringify` + `fs.writeFileSync`) — PowerShell `ConvertTo-Json` adds a UTF-8 BOM that breaks the startup seed
- **Rolling environment stats** are computed at prediction time from the local DB and injected into the Claude prompt automatically — this powers the `OVER_RATE_GATE` and dynamic May bias correction (§3.6, §3.9 of the framework)
- Allowed models: `claude-opus-4-6` (prediction default), `claude-sonnet-5` (analysis/extraction default), `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`

**`public/app.js`** — Analyzer page. Two input modes converge at `/api/predict`:
- Screenshot upload → `/api/analyze`
- Paste/JSON text → `/api/parse-text` (AI) or direct load (valid JSON)

`normalizeGameData()` normalizes pasted JSON regardless of schema variant — handles flat, `game_info`-wrapped, `starting_pitchers`/`team_recent_form`/`betting_odds`/`betting_lines` keys, team names as objects `{city, name}`, and numeric stat fields. Applied at all entry points before `showResult()`.

**`public/history.js`** — Paginated history table with inline result grading modal. Flag Performance Panel (`/api/flag-stats`) shows live ML/O/U accuracy per rule code, auto-refreshes every 30 s.

**`public/patterns.js`** — Chart.js dashboard; `/api/pattern-analysis` (10 SQLite aggregate queries) → 9 charts + algorithmically derived pattern cards.

**`public/combos.html`** — Static reference page (no JS file, inline scripts) showing the MC1–10 / OC1–10 / FD1–10 empirical combo codes, team 2-flag combos, and the weekly trend/O/U diagnostic panel. Served at `/combos`.

**Database — `predictions.db`** (SQLite via `better-sqlite3`):
- Single `predictions` table. Prediction fields on save; result fields (`actual_home_score`, `ml_correct`, `ou_correct`, etc.) via `/api/result/:id`
- Extra JSON array columns added via idempotent `ALTER TABLE` migrations at startup: `combo_hits`, `fade_signals`, `team_signals`, `team_combo_signals`, `bw_signals` — store the MC/OC/FD/BC/BW signal codes fired for each prediction
- `predictions-export.json` committed to git — auto-seeds an empty DB on Railway startup. **Never commit this file from a local `syncExportFile()` write** — always run `npm run sync-railway` first.

**Sync scripts:**
- `sync-from-railway.js` — fetches Railway `/api/export-all` → writes `predictions-export.json` only (does NOT touch local DB)
- `sync-local-db.js` — fetches Railway → upserts into local `predictions.db` (new rows inserted; result fields updated via `ON CONFLICT DO UPDATE`)
- `push-to-railway.js` — reads all local rows → POST to Railway `/api/import` with `x-import-secret` header (uses `IMPORT_SECRET` from `.env`)
- `sync-scores-to-local.js` — fetches actual MLB scores and writes them back to local DB (`actual_home_score`, `actual_away_score`, `ml_correct`, `ou_correct`)
- `export_with_rules.js` — one-off utility that re-exports `predictions-export.json` with `active_flags` and combo fields backfilled from rules logic (not part of normal workflow)

**Deployment — Railway:**
- Auto-deploys on push to `master`
- `DB_PATH` env var → `/var/data/predictions.db` (persistent volume)
- `IMPORT_SECRET` env var → `mlb2026import` (required by `/api/import`)
- **Pre-commit workflow (mandatory — see Section 11):**
  1. `npm run push-railway` — push any local-only rows up first
  2. `npm run sync-railway` — refresh `predictions-export.json` from Railway
  3. `git add … && git commit && git push`

---

# MLB Game Predictor — Analytical Framework v3.16
...
> v3.16 — BC/BW joint analysis (2026-07-28): new joint ML+O/U outcome indicator layered on top of the existing combo/fade system, from a 1239-game analysis of games where both markets land correctly together (Both-Correct, "BC") or fail together (Both-Wrong, "BW"). 7 BC codes added to `combo_hits[]` (a third code family alongside MC/OC) representing elevated conviction to bet BOTH markets — two (BC2, BC4) have zero recorded both-wrong games across n=19–21 each. 7 BW codes added to a new dedicated `bw_signals[]` array/DB column — these are hard double-PASS overrides, not fades: when a BW code fires, both ML and O/U bet sizing are overridden to "PASS BOTH MARKETS" regardless of what else fired, since the joint failure isn't clean enough to bet the reverse side confidently on both markets. A surging away starter (`away_surge`) anchors 6 of the top 7 BW combos, confirming it as the framework's most damaging recurring blind spot from yet another independent angle.
> v3.15 — 1363-game combo/fade refresh (2026-07-28): MC1–10 (ML combos), OC1–10 (O/U combos), and FD1–10 (reverse ML signals) re-derived from the 1363-game dataset (+134 games since v3.14/1229g); new **Reverse O/U signal family** added (FDOU1–10, §COMBO SIGNAL EVALUATION in the prediction system prompt) — a distinct set of combos where the model's O/U call itself (not the ML pick) should be flipped, surfaced via the existing `fade_signals[]` array. Two zero-both-wrong ML+UNDER dual plays identified (MC2, MC6). Two ML fades (FD2, FD4) cross-referenced as also being top-5 both-wrong combos — flagged for PASS over confident reversal. FDOU5/FDOU6 (GVI<35+TMF) identified as a rare case where the model's UNDER call fails and OVER is the correct reverse, the opposite direction from most other O/U fades and worth tracking against the §3.13 base-rate OVER-bias correction rather than as a contradiction of it.
> v3.14 — 1229-game full-season flag audit (2026-07-18): structural OVER/UNDER base-rate miscalibration identified and corrected (model predicted OVER 70.1% vs actual 49.6% — see new §3.13); WP-Override B promoted from weak secondary signal to primary ML driver (47%→60.8% ML, n=102); HFCF ML rate recalibrated (85.7%→59.5% ML, n=126, "best predictor" claim dropped); RCF+Slumping OVER demoted to informational-only (65%→49.6% OU, n=113 — signal decayed to coin flip at the same sample size); GVI≥90 hard pass softened to caution pending re-test of the exact triple condition (40.9%→57.1% ML at n=63 for GVI≥90 alone — inverted); GVI≥65 May+ catalyst gate stat updated (44.9%→53.4% OU, n=423 — recovered but catalyst requirement retained); OU-B and Dome flags marked for isolated re-test (OU-B 50.2% OU at n=620 — coin flip at the largest sample in the system; Dome 44.0% OU at n=284 — possible sign/direction error under investigation); R14 away-ace WP-flip implementation bug identified and fixed (flat ±10% adjustment could fail to cross over when the pre-R14 home-favoring gap was ≥20%, leaving grading inconsistent with the "bet away" recommendation).
...
> **Flag:** `"SINGLE_RED_UNAV: [home/away] SP RED_unavailable — O/U Lean [direction] ⚫ EXTREME RISK, no bet (26.1% BW vs 4.8% BC, most extreme gap — §3.12)"`
...
**RCF + Slumping Combo OVER Signal (v3.0 — DEMOTED v3.14, 1229-game audit):** When RCF fires on a pitcher AND that pitcher is also Slumping (RED > +1.5), this was historically treated as a **strong OVER signal** — 65% OVER hit rate across 17 qualifying games in the original 113-game sample. **At full-season scale, the exact same n=113 subset now measures 49.6% OU — a coin flip.** This is not sample dilution; it's the same sample decaying, meaning the signal has genuinely eroded rather than simply needing more data. **Demoted to informational-only** — do not escalate confidence and do not treat as a standalone OU-A trigger. Log the flag for tracking but require a second independent signal (OU-B, R11 Slumping SP, GVI, etc.) before it contributes to bet sizing. Pending re-validation on fresh post-audit data before any re-promotion.
> **Flag:** `"RCF+Slumping OVER — informational only (49.6% OU at n=113, collapsed from 65% — v3.14 demotion, do not escalate confidence)"`
> **Note:** This applies to the SP with the RCF flag. Prior guidance to escalate to Strong OVER when both SPs qualify is suspended pending re-test.
...
**OVER_RATE_GATE (v3.7 — 437-game empirical):** Before issuing any OVER bet in **May+**, compute the rolling 5-day actual OVER rate (% of recently graded games where actual_total > ou_line). Detects scoring environment compression that GVI/PVS mechanics cannot. Evidence: May 8–10 rolling rate = 36% while model called OVER 84% of games — 24 of 26 both-wrong games had OVER predictions that went UNDER.

| Rolling 5-day OVER Rate | Action |
|------------------------|--------|
| ≥ 45% | Normal — proceed with standard OVER signals |
| 42–44% | **OVER_CAUTION:** −15 confidence on all OVER predictions; require RED mismatch > 2.0 between SPs to confirm OVER direction |
| < 42% | **OVER_SUPPRESSED:** GVI-only OVER calls blocked entirely; OVER eligible only with Slumping SP RED > +1.5 OR Wind OUT > 12mph + temp > 65°F OR line 9.0–10.0 (R2) |
| < 38% | **OVER_HARDBAN:** −25 confidence; OVER bet requires 2+ named primary signals; GVI from PVS alone = PASS |

> **Flag:** `"OVER_RATE_GATE: 5-day OVER rate [X]% → [NORMAL/CAUTION/SUPPRESSED/HARDBAN]"`
> **April exempt:** Gate only applies May 1 onward. In April, use existing April caps.
...
**High-Line April OVER Ban (v3.2 — NEW, v3.6 updated):** When game date is **April 1–30** AND O/U line **≥ 9.5** → **Lean [GVI/subsidiary direction] 🔴 HIGH RISK — no active OVER bet** (36% hit rate). OVER bet is suppressed; direction is still output per §3.12. The market at ≥9.5 fully prices in the offense; calling OVER produces losses. Flag: `"HIGH_LINE_OVER_BAN: April line ≥9.5 → Lean [direction] 🔴 HIGH RISK, no OVER bet (36% OVER, §3.12)"`
...
**Heavy Favorite Confirmation Flag (HFCF — RECLASSIFIED v3.8, RECALIBRATED v3.14):** Either team's final win probability reaches **68% or higher**. Previously a caution flag (−20 confidence), then **reclassified as ML CONFIRMATION** at v3.8 on a 534-game/n=14 sample showing 85.7% ML. **1229-game audit (n=126) measures 59.5% ML** — still solidly above the ~55% season baseline and a genuine positive signal, but the n=14 sample was far too small to support the original "single best ML predictor in the dataset" claim; that language is retired. New rule: HFCF active → **ML bet confirmed at $75 (Tier 1)**, using **59.5%** as the quoted rate going forward. Still applies −20 to **O/U only** (OU remains weak in high-WP games — unreliable). Flag: `"HFCF_CONFIRM: WP≥68% — ML Tier 1 $75 (59.5%, n=126 — v3.14 recalibrated). O/U caution −20."`
...
| High-line OVER lean (v3.2, v3.6) | O/U line **9.0–9.4** → OVER (Low confidence). O/U line **≥9.5** → Lean [GVI direction] 🔴 HIGH RISK — no active OVER bet (36% hit rate). See §3.6 + §3.12. |
| 7.5–7.9 UNDER ban (v3.1, v3.6) | O/U line 7.5–7.9 → Lean UNDER 🔴 HIGH RISK — max $25 lean, no standard UNDER bet (27.3% hit rate). Per §3.12. |
...
- **May onward (dynamic — v3.7):** Base +2.0 runs. **Exception:** When rolling 7-day actual avg total < 8.5, reduce to **+1.5 runs** (compressed environment detected). Empirical: May 8–10 avg actual = 7.8 with avg line 8.41 — model was projecting 0.6 runs too high even with +2.0. Lower bias makes the ≥2.0 gap requirement harder to satisfy, naturally suppressing OVER bets.
...
> **⚠️ DOME DIRECTION UNDER INVESTIGATION (v3.14, 1229-game audit):** All dome games combined (n=284, both P20 and P21 populations plus untriggered dome games) are landing at **44.0% O/U — below breakeven**, a large enough sample that it should not be dismissed as noise. This raises a real possibility that the dome flag is currently nudging predictions toward OVER (on "controlled environment, no wind suppression" reasoning) when the full-season data says it should trend UNDER or stay neutral — i.e. a possible sign/direction error rather than just a decayed signal. **P20/P21 below are left unchanged pending a direct code-level investigation of which direction the dome flag is currently nudging predictions**, but treat both as reduced-confidence / secondary-only until that investigation is complete.
> **Flag:** `"DOME_RETEST: combined dome O/U at 44.0% (n=284) — possible sign error under investigation, treat as reduced-confidence (§3.13)"`

**`P20_DOME_OVER` — Dome Stadium OVER (NEW v3.3):**
- Trigger: Game in dome/indoor stadium AND model direction is OVER.
- Effect: **67% OVER hit rate (6/9 games) in domes** at original small-sample size. Dome eliminates weather suppression — without cold temperatures and wind-in, April offensive conditions persist indoors. **Caution:** the broader n=284 dome sample above reads below breakeven — do not treat this small-sample figure as still current without a second signal.
- Output: `"P20_DOME_OVER: Dome stadium OVER — reduced-confidence pending dome re-test (see DOME_RETEST, §3.13)"`
...
- Effect: **BANNED — 37% UNDER hit rate (10/27 games) in domes** at original small-sample size. Dome removes all weather suppression factors. Without cold/wind floor, model's UNDER logic relies entirely on SP ERA — unreliable in April for sub-6-start pitchers.
- **Exception:** Both SPs confirmed ≥4 starts + sub-2.50 ERA → route to P1_MATCH (dual-ace dome = valid UNDER).
- Output: `"P21_DOME_UNDER_BAN: Dome UNDER without dual confirmed ace — BANNED (37% hit rate, n=27; see also DOME_RETEST, §3.13)"`
...
### 3.12 Never-Pass O/U Direction Policy (v3.6 — 2026-05-05)

> **Core Rule:** The model must ALWAYS output a directional lean (OVER or UNDER) for O/U predictions — never a bare "PASS." The **betting recommendation** may still be "no active bet" or a reduced stake, but the direction field is always populated. Ban rules and dead zones become **risk level escalators**, not direction eliminators.

> **Empirical basis:** 50 graded PASS games showed 46% OVER / 54% UNDER (May actuals: 58.7% UNDER, n=46). Big OVER misses (5+ runs above line) had GVI 37–88, confirming GVI remains useful for lean direction even when primary signals are banned. Eliminating the PASS output captures this directional information without implying a bet should be placed.

#### Step 1 — Determine Lean Direction (when primary signals result in no clear direction)

After completing OU-A through OU-F, if the result is a suppressed/banned direction, apply in priority order:

1. **GVI ≥ 65** → Lean OVER (R7: 58.9% OVER at GVI 65+)
2. **GVI < 35** → Lean UNDER (GVI clearly suppressive — note: GVI<35+UNDER *bet* ban still applies; lean is informational only)
3. **GVI 35–65 (dead zone)** → use subsidiary signals to determine lean:
   - R11 Slumping SP active (RED > +1.5 on either pitcher) → Lean OVER
   - Wind OUT > 8 mph (even if cancelled by ace veto) → Lean OVER
   - Wind IN > 8 mph → Lean UNDER
   - Hitter's park + temperature ≥ 65°F → Lean OVER
   - Pitcher's park + temperature < 55°F → Lean UNDER
   - Park factor alone (no temperature signal) → Lean per park classification
   - **May+** with no other signal → Lean UNDER (May actual: 58.7% UNDER, n=46)
   - **April** with no other signal → Lean OVER (April base rate: 59.4% OVER)

#### Step 2 — Assign Risk Level

Evaluate all ban/suppression rules that fired; assign the **highest applicable tier**:

| Risk Level | Symbol | Trigger Conditions | Bet Size |
|-----------|--------|--------------------|---------|
| EXTREME RISK | ⚫ | SINGLE_RED_UNAV OR R1 (zero signals) OR BOTH_XFIP_BLIND OR P4_VETO OR P19_PIT OR GVI<35 (UNDER) | **Lean only — no bet. Display for tracking.** |
| HIGH RISK | 🔴 | R12 dead zone (conf 55–65) OR DUAL_PVS_SKIP OR BOTH_RED_UNAVAIL OR HIGH_LINE_OVER_BAN (April ≥9.5) OR GVI dead zone (35–65) as sole basis for direction | **Max $25 lean** — informational stake only |
| MODERATE RISK | 🟡 | Gap 1.0–1.9 runs (below 2.0 threshold) OR single suppression flag OR BSS_LINE_CAP OR April cap limiting | **$25–$37.50 lean** |
| STANDARD | 🟢 | All gates pass; primary signal active; gap ≥ 2.0 | **Normal sizing per pattern tier** |

#### Step 3 — Output Format (replaces bare "PASS")

Every O/U prediction must show direction + risk annotation. Examples:
- `OVER (Lean — ⚫ EXTREME RISK — tracking only: SINGLE_RED_UNAV + no primary signal)`
- `UNDER (Lean — 🔴 HIGH RISK — max $25: R12 dead zone, conf 58 in 55–65 range)`
- `OVER (Lean — 🟡 MODERATE RISK — $25 lean: gap 1.4 runs < 2.0 threshold)`
- `OVER (Moderate — 🟢 STANDARD — $75: line 9.0–10.0 OVER Pattern D)`

> **GVI<35 UNDER note:** The ban on GVI<35 UNDER *bets* remains fully active (7/7 = 100% failure). The lean of UNDER is still displayed with ⚫ EXTREME RISK — tracking only. Distinction: directional lean ≠ bet recommendation.

> **R1 (no signal) lean:** When zero OU signals fire, GVI-based direction from Step 1 is the lean. Marked ⚫ EXTREME RISK (16.3% accuracy). No bet placed. Prior "no direction output" rule is retired for tracking purposes.

> **JSON field behavior:** The `ou_prediction` JSON field must always be "OVER" or "UNDER" — never "PASS". Add `ou_risk_level` (STANDARD / MODERATE RISK / HIGH RISK / EXTREME RISK) and `ou_bet_eligible` (true/false) fields to the output schema.

---

### 3.13 Structural Base-Rate Correction (v3.14 — 1229-game full-season audit, 2026-07-18)

> **⚠️ This is the single highest-priority finding in the v3.14 audit.** Every rule in §5 assumes the model's OVER/UNDER calls are unbiased before rule-level adjustments are applied. They are not.

**The finding:** Across the full 1229-game dataset (1124 games with both a prediction and a graded actual total), the model predicted **OVER on 70.1%** of all games (788/1124) while the actual outcome split almost exactly 50/50 — **49.6% OVER / 50.4% UNDER**. This is a **+20.5 percentage point structural over-prediction bias**, and critically it is **not** a recent drift — the same skew holds across the entire season, not just the most recent 6 weeks that the rolling `OVER_RATE_GATE` (§3.6) was designed to catch. The model's baseline OVER/UNDER prior is miscalibrated by roughly 20 points system-wide.

**Why this matters:** `OVER_RATE_GATE` (§3.6, added v3.7) and the R15/R16 May+ rules were built as *temporary emergency patches* triggered only when a rolling 5-day window drops below 45%. Because the bias is present all season, treating it as an emergency condition means the correction is absent whenever the rolling window happens to read normal — which is most of the time, since the bias is baked into the baseline rather than concentrated in bad stretches.

**Mandatory rule change:** `OVER_RATE_GATE` and its associated confidence deductions (ORC/ORS/ORHB/OGCF/MCOW, §6) are hereby reclassified from **rolling-window emergency patch** to **permanent structural correction**, active in every game regardless of the current rolling-window reading:
- Every OVER lean — in any GVI band, in any month — must be treated with the presumption that the model's own prior is over-weighted toward OVER by ~20pp. A GVI≥65 reading or an OU-B environmental signal is **necessary but not sufficient** justification for an OVER call; require at least one *named, independently-verified* catalyst (Slumping SP RED>+1.5, confirmed Wind OUT>12mph+temp>65°F, both offenses wRC+>105, or line 9.0–10.0) before issuing a standard OVER bet, in every month, not just May+.
- In the GVI 35–65 dead zone (§5 OU-E) and the R1/OU-F no-signal case, the default lean when no other signal is present shifts to **UNDER**, not month-dependent OVER/UNDER defaults — the season-long data no longer supports an April-OVER-default assumption at the base-rate level, even though specific April rules (high-line OVER lean, etc.) that are grounded in their own named signals remain unchanged.
- This does not ban OVER bets — R2 (line 9–10 OVER), P12–P14/P18/P20/P25, RCF+OVER (where still valid), and Wind OUT with catalyst remain live, evidence-backed OVER signals. It bans **unsupported** OVER calls that rely on GVI or PVS alone.

**Two flags under active re-test as likely contributors to the bias (see below and §5 OU-B / §3.10 Dome patterns):** OU-B (environmental signal) is the single largest-sample flag in the framework (n=620) and is landing at **50.2% OU** — essentially no edge — despite being one of the two pillars (with OU-A) that the O/U logic is built on; if OU-B is frequently the trigger for an OVER call and carries no real predictive value, that alone could explain much of the +20.5pp gap. Dome games (n=284) are landing at **44.0% O/U**, below breakeven, suggesting domes may be nudged toward OVER when the data says they should trend UNDER or neutral. Both are flagged for **isolated re-test after this base-rate correction is applied** — some of their poor showing may be inherited from the same season-long skew rather than being a flaw unique to either flag. Do not fully retire either flag yet; treat both as reduced-confidence / secondary-only pending that re-test.

> **Flag:** `"BASE_RATE_CORRECTION: season OVER bias +20.5pp (predicted 70.1% vs actual 49.6%, n=1124) — OVER_RATE_GATE active permanently, not rolling-window-gated"`
...
**WP-Override B — Home Fortress Lock (DOWNGRADED v3.5, PROMOTED v3.14):**
An active Home Fortress team (home win% ≥ .650) faces an opponent with a road winning percentage below .500.
→ Award the home team **+10% win probability** from baseline (raised from the v3.5-downgraded +5%, and above the original +10%'s prior evidentiary basis). **1229-game audit reversal:** the v3.5 downgrade was based on an n=87 external sample reading 47% ML. At full-season scale (n=102), WPB now measures **60.8% ML — the 3rd-strongest individual flag in the entire framework**, behind only GVI<35 and WPA, and it has been the dominant driver in several recent top-10 ML combos (see COMBO SIGNAL EVALUATION, MC8/OC3/OC5). **WPB is promoted from "weak secondary signal" to a primary ML driver, on par with WP-Override A** — it is no longer barred from driving bet sizing on its own.
→ Flag: "WP-Override B fired (60.8% ML, n=102 — v3.14 promoted to primary)"
...
> **PDCF O/U Block (v3.8):** PDCF + conf < 45 → **both ML and O/U = PASS**. 534-game data: PDCF OU = **32.6%** (n=43), ML = 44.4% — both markets fail simultaneously. The conflict is unresolvable regardless of direction. PDCF + conf ≥ 45 → both markets eligible with standard sizing (marginal). Flag: `"PDCF_OU_BLOCK: conf<45 — ML PASS + O/U PASS (32.6% OU, 44.4% ML — both below breakeven)"`
...
- Check HFCF (≥68%) → **ML CONFIRMED at Tier 1 $75** (85.7% ML, n=14 — reclassified v3.8); apply −20 to O/U only
- Check MCF (contradicts betting line favorite) → flag and apply −25 confidence
- **GVI≥80 WP Reduction (v3.8):** When GVI≥80, reduce WP contribution from all PVS-related signals (HVIF, VMF) by 50%. GVI≥80 ML = 49.4% — high GVI environment distorts WP through PVS volatility. Apply: HVIF deduction halved (−10→−5 for WP purposes), VMF halved. GVI still applies to O/U direction unchanged. Flag: `"GVI80_WP_REDUCTION: GVI≥80 — PVS WP bonuses halved (49.4% ML without reduction)"`
- **Conf<50 ML Gate (v3.8):** Conf < 50 + ML bet → require **WP gap ≥ 20%** to proceed. If WP gap < 20% at conf < 50 → ML = PASS. Evidence: 76% of May 17+ ML losses had conf < 50; conf < 50 without WP gap = 43.6% ML (below breakeven). The no-edge zone (47–53%) catches close games but not 53–60% WP at low confidence. Flag: `"CONF_ML_GATE: conf<50 + WP gap<20% → ML PASS (43.6% without quality advantage)"`
...
- **Condition 3 — Both Slumping (REVISED THRESHOLD v3.5):** Combined RED (homeRED + awayRED) > **+1.0** when BOTH SPs are trending slumping direction (both RED positive) → **Lean OVER** + WP equalize −8%. *Threshold lowered from "both individually >+1.5" to "combined >+1.0"* — external validation: combined >+1.0 = 54% OVER; prior "both >+1.5" collapsed to 30% (market priced extreme slumping into the line). Escalate to Strong OVER if either team's 15-day wRC+ > 108. Flag: "Both SP Slumping — WP Equalized".
  > **BSS Line Cap (v3.2):** BSS OVER fires only when line ≤ 8.5. Line > 9.0 → −20 confidence, cap at **Lean OVER (🟡 MODERATE RISK — no standard bet)** per §3.12. Flag: `BSS_LINE_CAP`
  > **BSS Line Cap (v3.2):** BSS OVER signal only fires (and escalates to OVER direction) when the **O/U line is ≤ 8.5**. When line > 9.0, the market has already priced in the expected pitching volatility — BSS edge disappears at high lines (41% hit rate at line >9.0). Apply −20 confidence if BSS fires but line > 9.0; output Lean OVER (🟡 MODERATE RISK) per §3.12 Never-Pass Policy — no standard bet placed. Flag: `"BSS_LINE_CAP active — line >9.0, BSS signal suppressed (-20 conf), Lean OVER §3.12"`
- **Condition 4 — Slumping Home SP + Heat + Hitter's Park (v3.2 NEW):** Home SP is Slumping (RED > +1.5) AND temperature ≥ 75°F AND park classification = Hitter's Park → **Strong OVER** — **87% OVER hit rate**.
- **Condition 5 — Home SP Surging (RED < −1.0) → O/U UNDER lean (v3.8 NEW):** Home SP RED < −1.0 AND ≥5 confirmed starts AND Gate C met (ERA < 2.50) AND GVI < 90 → **Lean UNDER** (64.0% OU in May, n=25). Mechanism: surging home SP suppresses away team scoring → total stays lower. Apply as O/U UNDER lean independently of WP signal (home surge RED is separately a −10 confidence deduction on home ML, now also dual-use as O/U UNDER). Requires: confirmed RED (not RED_thin), no OVER_RATE_GATE suppression in opposite direction. Flag: `"HOME_SURGE_UNDER: home SP RED<−1.0 + Gate C met → O/U UNDER lean (64.0% OU, May n=25)"` This combo overrides the April High Confidence Hard Cap: escalate to **High OVER** even in April when all three conditions are confirmed. Mechanism: slumping pitcher + heat-inflated ball carry + offensive park = structural OVER condition. Flag: `"SLUMP+HEAT+PARK: home SP slumping + ≥75°F + hitter park → Strong/High OVER (87%)"`
- **Veto (per-condition):**
...
> **⚠️ UNDER RE-TEST (v3.14, 1229-game audit):** OU-B is the single largest-sample flag in the entire framework (n=620) and is currently landing at **50.2% OU — a coin flip, essentially no predictive value at scale**, despite being one of the two pillars (with OU-A) the whole O/U logic is built on. This is very likely a major contributor to the season-long OVER bias documented in §3.13 — if OU-B (wind out, hot conditions) is frequently the trigger for calling OVER and it isn't actually predictive, that alone could explain much of the +20.5pp gap. **Do not treat OU-B in isolation as sufficient justification for a standard bet** until it has been re-tested independently of the §3.13 base-rate correction (some of its poor showing may be inherited from the same season-long skew rather than a flaw unique to OU-B). Continue applying the sub-rules below (wind tiers, Wind-Ace Interaction, Wrigley, Coors) as-is, but require a second independent signal before sizing a standard bet off OU-B alone.
> **Flag:** `"OU_B_RETEST: OU-B environmental signal at 50.2% OU (n=620) — coin flip at scale, requires second signal for standard bet (§3.13)"`
...
- GVI > 75 (no ace veto) → **Strong OVER** (67% hit rate — requires no ace xFIP ≤ 3.25 veto; **May+ catalyst required**, see below)
- GVI 65–75 → **Lean OVER** (61% hit rate — **May+ catalyst required**, see below)

> **May+ GVI OVER Catalyst Gate (v3.7):** In May and beyond, GVI ≥ 65 OVER direction requires at least ONE confirmed catalyst beyond GVI itself: (a) Slumping SP RED > +1.5, (b) Wind OUT confirmed > 12mph + temp > 65°F, (c) both offenses 30-day wRC+ > 105, OR (d) line 9.0–10.0 (R2 pattern). GVI elevated purely by PVS volatility (pitchers accumulating starts) with no named catalyst → route to **PASS (OVER_GVI_CATALYST_FAIL)** in May+. Evidence: GVI≥65 OVER dropped from 56.6% to **44.9%** in May 4+ — below breakeven. The same GVI values produced by more PVS data in May do NOT predict scoring explosions the way they did in April.
> **Flag:** `"OVER_GVI_CATALYST_FAIL: GVI≥65 but no May+ catalyst — PASS (44.9% hit rate, below breakeven)"`

- GVI < 35 + UNDER → **PRE-GATE HARD BAN** — 294-game data: 7/7 = **100% failure**, avg actual 13.7 runs. This is the most dangerous rule in the system. The prior "Strong UNDER 69%" figure is completely inverted at scale. **NEVER bet UNDER when GVI < 35.** GVI < 35 → Lean OVER or Lean UNDER (⚫ EXTREME RISK) only — no standard bet on UNDER. Per §3.12, direction is still output for tracking. Flag: `"GVI<35_UNDER_BAN: hard ban — 7/7 failure, avg 13.7 actual runs"`
- **GVI 35–65 → Dead Zone — Lean [direction via §3.12 subsidiary signals] (🔴 HIGH RISK — max $25; no active bet based on GVI alone)**. Do NOT place a standard O/U bet based on GVI alone in this range. Only override with a primary signal: P10 ≤ 6.5, RCF+Slumping (65%), or Wind OUT > 15mph (78%). Empirical: 113-game dataset confirms GVI 35–65 produces no meaningful directional edge. Apply §3.12 Step 1 subsidiary signals to determine lean direction. *(capped at Moderate confidence in April per §3.6)*
...
→ **Default: Lean [direction via §3.12 Step 1] (⚫ EXTREME RISK — tracking only, no bet).** Apply §3.12 Never-Pass Policy. The 59.4% April OVER stat is a population average for games where a signal already fired — it is NOT a bet trigger. R1 (16.3% accuracy with no signal) = no active bet.
→ Per §3.12 Step 1: if GVI ≥ 65 → Lean OVER; if GVI < 35 → Lean UNDER; GVI 35–65 → use subsidiary signals (park factor, temperature, wind) to determine lean. Output direction always; stake never in this scenario.

**High-Line Lean (v3.2):** See §3.6 — lines **9.0–9.4** lean OVER (Low confidence). Lines **≥9.5** → Lean [GVI/subsidiary direction] (🔴 HIGH RISK — max $25, no standard bet; OVER banned at 36% in April at ≥9.5). Output direction always per §3.12; suppress active OVER bet only.

**Low-Line UNDER Ban — 7.5–7.9 (v3.1 — NEW):** When O/U line is **7.5–7.9** AND model direction is UNDER in April → **Lean UNDER (🔴 HIGH RISK — max $25 lean, no standard Under bet)**. Hit rate: 3/11 = **27.3%**, avg actual total = **10.3 runs**. The market price of 7.5–7.9 already reflects suppression; no standard Under bet. Direction still shown per §3.12. This supersedes the prior Low-confidence cap.
...
Zero OU signal flags AND no Slumping/Surging flags active = O/U accuracy collapses to **16.3%**. Catastrophic — well below the 52.4% breakeven. **Never place an O/U bet when no signal flag is active.** Per §3.12, output a directional lean (GVI-based or subsidiary signal via §3.12 Step 1) marked ⚫ EXTREME RISK — tracking only, no bet. Conf ≤49 with no named signal = Lean [direction] (⚫ EXTREME RISK) — not a bet. Hard stop on any stake: no exceptions in any month.
> **Flag:** `"R1_NO_SIGNAL: zero O/U signal flags — Lean [direction] ⚫ EXTREME RISK, no bet (16.3%, §3.12 lean-only)"`
...
**R7 (UPDATED v3.7 — 437-game, RE-MEASURED v3.14) — GVI 65+ OVER: viable in April (58.9%), recovered in May at scale (53.4%) but catalyst gate retained**
Historical: GVI≥65 OVER = 58.9% (viable). May 4+ reality read 44.9% at the time R7 was written (below breakeven), driving the May+ Catalyst Gate. **1229-game audit (n=423): this has recovered to 53.4% OU — above breakeven.** This recovery should be read alongside the season-long OVER base-rate bias documented in §3.13 — some of it may reflect the general over-prediction pattern rather than a fully recovered standalone signal, so **the May+ Catalyst Gate is retained rather than relaxed** pending a test of the catalyst-gated subset versus the non-gated subset in isolation. **Rule (unchanged):** GVI ≥ 65 → UNDER still hard-banned (0.0%, n=4); ML still skip (50% coin flip). OVER: eligible only when **May+ GVI Catalyst Gate passes** (see OU-E) — Slumping SP, Wind OUT >12mph+temp>65°F, both wRC+>105, or line 9–10. Without catalyst in May+: route to PASS, not OVER.
> **Flag:** `"R7_GVI65: GVI≥65 — UNDER BANNED (0%). ML skip (50%). OVER: catalyst required in May+ (53.4% OU at n=423 — recovered but gate retained pending isolated re-test, §3.13)."`

**R8 (UPDATED v3.8 — MCF refined with WP gap condition) — MCF active = ML banned + refined Slump exception**
MCF ML ban for GVI-only games remains fully active (43.1% ML — never bet). OVER→UNDER flip retained. **Refined v3.8 (534-game):** MCF + Slumping SP + WP gap ≥ 10% → **ML $75** (54%+ ML — genuine SP-form edge justifies overriding market). MCF + Slumping SP + WP gap < 10% → **ML $37.50** reduced stake (some edge but too close). MCF + no Slumping SP → ML ban maintained. For O/U: MCF active + OVER direction → flip to UNDER. MCF + UNDER → escalate UNDER confidence.
> **Flag:** `"R8_MCF_REFINED: MCF+Slump+WP≥10% → ML $75 | MCF+Slump+WP<10% → ML $37.50 | MCF+no-slump → ML BANNED. OVER flipped UNDER."`
...
Dead zone **extended from 55–60 to 55–65**. 294-game finding: conf 60–65 cluster is elevated in wrong games (15.9%) vs correct games (6.5%). **R10 ("conf 60–64 = O/U sweet spot 63.6%") is retired** — superseded by the 294-game reality. Valid O/U betting zone is now **conf 50–55 only** (below the dead zone) plus high-confidence Tier 1 signals where applicable. **Treat conf 55–65 for O/U as 🔴 HIGH RISK — output Lean [direction] (max $25) per §3.12; no standard O/U bet.** ML at conf 55–65 still eligible per normal rules.
> **Flag:** `"R12_DEAD_ZONE: Conf 55–65 (extended) — Lean [direction] 🔴 HIGH RISK max $25, no standard O/U bet. ML still eligible. R10 retired."`

**R15 (NEW v3.7 — 437-game) — OVER_RATE_GATE: rolling 5-day OVER rate < 42% = block GVI-only OVER bets**
May 8–10: rolling 5-day actual OVER rate hit 36% while model called OVER 84% of slates. OVER prediction accuracy collapsed to 41.8% (from 54.6%). Root cause: GVI/PVS mechanics calibrated on April data fire at the same rate in May, but scoring environment has compressed. **Rule:** Apply OVER_RATE_GATE (§3.6) every time any OVER direction is considered in May+. Compute: count of graded games last 5 days where actual_total > ou_line ÷ total graded. If rate < 42%: suppress GVI-only OVER. If < 38%: require 2+ primary signals for any OVER bet. UNDER accuracy improved to 63.6% in May — shift staking toward UNDER when gate is active.
> **Flag:** `"R15_OVER_RATE: Rolling 5-day OVER rate [X]% — [NORMAL/CAUTION/SUPPRESSED/HARDBAN] applied"`

**R16 (NEW v3.7 — 437-game) — Conf 50–55 + OVER in May requires 2 named signals (not 1)**
Conf 50–55 + OVER dropped from 59.3% to **31.2%** in May 4+ — the most reliable confidence zone polluted by directional OVER bias in a compressed scoring environment. Root cause: conf 50–55 + UNDER still hits ~60%+; conf 50–55 + OVER in May = 33% (destructive). **Rule:** In May+, conf 50–55 + OVER direction requires **≥ 2 named O/U signals** (e.g. Slumping SP + Wind OUT, or OU-A + OU-B). Conf 50–55 + UNDER remains eligible with 1 named signal (unchanged). This restores the confidence zone as a reliable UNDER filter without blocking OVER when genuinely supported.
> **Flag:** `"R16_MAY_CONF_FILTER: May conf 50–55 + OVER — requires 2 named signals (33% with 1 signal only)"`

**R14 (UPGRADED v3.8 — AWAY_ACE_HARD) — Surging away SP (≥5 starts confirmed): flip ML to away + adjust WP + skip all O/U**
534-game data: away surge RED<−1.0 + UNDER = **35.0% OU** (consistent loss). Away surge + OVER = **51.7% OU** (near breakeven). Both O/U directions fail. **Rule (three parts):**
1. **ML:** When away SP RED < −1.0 AND ≥5 confirmed starts → route ML to **away team unconditionally**. Flip direction if model backs home. Overrides WPB, TMS home, Home Fortress (not WP-Override A).
2. **WP output adjustment (critical for UI consistency):** After flipping ML to away, also subtract 10 from `home_win_pct` and add 10 to `away_win_pct` in the JSON output, then renormalize to sum to 100. This ensures the displayed win probability bar is **consistent with the ML recommendation** — the away team must show a higher WP when R14 overrides to away. Without this, the UI shows "PHI 52%" but recommends CIN — a visible contradiction.
3. **O/U:** Skip all O/U bets on that game (both directions unreliable). ML signal only.
> **Flag:** `"R14_AWAY_ACE_HARD: Away SP surging (RED<−1.0, ≥5 starts) — ML away unconditional + WP adjusted (−10% home, renormalized) + O/U SKIP."`

> **Implementation bug found and fixed (v3.14, 1229-game audit):** The audit found that when R14 fires, the model's stored win probability already leans away in 124/159 games (78%) and the underlying away-team win rate in these games is a genuine 53.5% — yet recorded ML grading accuracy was only **47.8%, below even the away team's own base win rate.** This was traced to the server-side reconciliation step that auto-corrects `home_win_pct`/`away_win_pct` when R14 fires but the model forgot to apply the mandated −10%/+10% shift: the prior implementation applied a **flat ±10-point adjustment**, which fails to cross the predicted winner over to away whenever the pre-R14 home-favoring gap was **≥20 points** (e.g. home 65/away 35 → home 55/away 45 after the flat fix — still home-favored, so grading scored it as a home pick even though the bet recommendation said away). **Fixed:** the reconciliation now forces `away_win_pct` to exceed `home_win_pct` by a real margin (≥6 points) regardless of the original gap size, and also checks `active_overrides` (not just `active_flags`/`betting_recommendation`) when detecting that R14 fired. The underlying 53.5% away-win signal itself was never in question — this was purely a grading/WP-consistency bug, not a data recalibration. Re-audit ML accuracy for this flag after this fix has been live for a meaningful sample.
...
- **HFCF active (WP≥68%) → ML Tier 1 $75** — **59.5% ML, n=126** (recalibrated v3.14 from the original 85.7%/n=14 claim — still above the ~55% season baseline, "best single ML signal" language retired)
- **WP-Override B (Home Fortress vs weak road team) → ML $75** — **60.8% ML, n=102** (promoted v3.14 from "weak secondary signal only" — now a primary ML driver, on par with WPA)
- **PWF active + any signal → ML 86%** (R14/R13) — highest alpha signal in system
- **WP gap ≥15% ML** → 63.0% (n=173) — primary ML signal
- **OUA + OUB + OVER** → **57.8% OU (n=109)** — elevated to Tier 1 (v3.8); largest reliable O/U sample
- **Line 8–9 + UNDER** → **60.6% OU (n=71)** — best O/U combo overall
- **UNDER prediction + home SP surge RED<−1.0** → 64.0% OU May (v3.8 dual signal)
- **MCF active + OVER → flip to UNDER** → 63.6% OU in May (R8 flip)
- **Conf 50–55 + named signal + UNDER** → reliable O/U zone
- UNDER line 8–9 with Slumping SP flag → **63.6%** (R6 + R11)
- RCF + OVER (§3.5) → **63.3%** — valid catalyst for OVER_RATE_GATE
- Slumping SP (home or away) present → **62%+** (R11)
- GOLDEN_CONDITION triple signal (OU-A + OU-B + RED mismatch >1.5) → gap ≥ 1.5
- Line 9–10 OVER + ≥1 signal active → **68.8%** (R2) — still valid with catalyst
...
- **SINGLE SP RED_unavailable** → O/U Lean ⚫ EXTREME RISK, no bet (26.1% BW vs 4.8% BC — most extreme gap; direction shown per §3.12)
...
- **MCF + ML → 50% coin flip** (R8 BAN) — full ML prohibition; MCF + OVER → flip to UNDER (R8 v3.7)
...
- **GVI≥65 OVER in May+ without catalyst → PASS** (R7/v3.7) — OVER_GVI_CATALYST_FAIL; gate retained despite the base rate recovering to 53.4% at n=423 (v3.14, pending isolated re-test — see R7)
- **OVER_RATE_GATE active (5-day rate <42%) + GVI-only OVER → suppressed** (R15/v3.7). **v3.14: this gate is now a permanent structural correction, not rolling-window-gated — see §3.13.**
- **Conf 50–55 + OVER in May with only 1 named signal → 33%** (R16/v3.7) — needs 2 signals
- **PDCF + conf<45 → both ML and O/U = PASS** (v3.8) — 44.4% ML, 32.6% OU (both markets fail)
- **Conf<50 + WP gap<20% → ML PASS** (v3.8) — 43.6% ML without quality advantage
- **RCF+Slumping OVER → informational only** (v3.14 demotion) — 49.6% OU at n=113, collapsed from the originally documented 65% at the same sample size; do not use as a standalone trigger — see §3.5
- **GVI≥90 + OVER + line≥9 → CAUTION, reduced stake, not a hard ban** (SOFTENED v3.14) — original 40.9% ML/45.0% OU was based on n=22 for the full triple condition; at n=63, GVI≥90 alone (not the full triple condition) now measures **57.1% ML — above breakeven, an inversion of the original finding.** The exact triple condition (GVI≥90 + OVER + line≥9) has not yet been re-tested at scale — until it is, treat as a caution (apply the existing GVI≥80 WP-reduction logic and −20 confidence) rather than an outright hard pass. See §6 GVI80_WPR.
- **Away surge RED<−1.0 + O/U → SKIP all directions** (R14 v3.8) — 35% UNDER, 51.7% OVER (both unreliable)
...
| Heavy Favorite Confirmation | HFCF (v3.8 reclassified, v3.14 recalibrated) | **ML +Tier1** / O/U −20 | WP≥68% → ML confirmed $75 (59.5%, n=126 — recalibrated from the original 85.7%/n=14); O/U still penalised −20 |
...
| OVER Rate Caution | ORC (v3.7) | −15 | Rolling 5-day actual OVER rate 42–44% — OVER confidence reduced; RED mismatch >2.0 required |
| OVER Rate Suppressed | ORS (v3.7) | −25 | Rolling 5-day actual OVER rate < 42% — GVI-only OVER blocked; named catalyst required |
| OVER Rate Hard Ban | ORHB (v3.7) | −30 | Rolling 5-day actual OVER rate < 38% — OVER requires 2+ primary signals; GVI alone = PASS |
| GVI OVER Catalyst Fail | OGCF (v3.7) | −20 | May+ game: GVI≥65 OVER with no named catalyst (Slumping SP / Wind+temp / wRC+>105 / line 9–10) — PASS |
| May Conf OVER Weak | MCOW (v3.7) | −20 | May+ conf 50–55 + OVER with only 1 named signal — requires 2 signals (33% with 1 signal) |
| PDCF O/U Block | PDCF_OUB (v3.8) | −30 O/U | PDCF active + conf<45 — O/U PASS (32.6% OU, both markets fail simultaneously) |
| Conf ML Gate | CONF_ML_GATE (v3.8) | ML PASS | conf<50 + WP gap<20% — ML PASS (43.6% ML without quality advantage) |
| GVI≥80 WP Reduction | GVI80_WPR (v3.8, softened v3.14) | WP adj | GVI≥80 — PVS-driven WP bonuses halved; GVI≥90+OVER+line≥9 = CAUTION + reduced stake (was hard pass at 40.9% ML/n=22 — GVI≥90 alone now measures 57.1% ML at n=63, inverted; full triple condition pending re-test, see Priority Checklist) |
| Away Ace O/U Skip | AWAY_ACE_SKIP (v3.8) | O/U PASS | Away SP RED<−1.0 ≥5 starts — skip all O/U directions (35% UNDER, 51.7% OVER — both unreliable) |
| Home Surge UNDER | HSU (v3.8) | O/U signal | Home SP RED<−1.0 + Gate C — O/U UNDER lean (64.0% OU May, n=25) |
...
5. **O/U Prediction** — ALWAYS output a direction per §3.12. Format: `DIRECTION (Confidence — X%) [Risk]`. Standard: `OVER (Moderate — 61%) 🟢`. Lean/banned: `OVER (Lean — ⚫ EXTREME RISK — tracking only: SINGLE_RED_UNAV)` or `UNDER (Lean — 🔴 HIGH RISK — max $25: R12 dead zone)`. Never output bare "PASS".
...
O/U DIRECTION: [OVER/UNDER always shown per §3.12 — never blank]
O/U RISK:  [🟢 STANDARD / 🟡 MODERATE RISK $25–37 / 🔴 HIGH RISK max $25 / ⚫ EXTREME RISK tracking only]
UNDER BET: [All gates pass → Bet $[size] [pattern tier] UNDER [line] / Lean [direction] [risk level] — Gate [X] failed: [reason]]
FINAL RECOMMENDATION: [Combined ML + O/U bet/lean summary. Always includes O/U direction + risk level. No "PASS" as final output.]
```
...
|-----------|----------|------|--------|
| P4_VETO or P8_BAN active | **Lean [direction] ⚫ EXTREME RISK** | — | No bet — hard ban; output GVI-based lean per §3.12; ML still eligible |
| P19_PIT_HOME_SKIP active | **Lean [direction] ⚫ EXTREME RISK** | — | PIT home O/U bet banned; lean direction shown; ML still eligible |
| BOTH_RED_UNAVAIL or DUAL_PVS_SKIP active (no primary signal) | **Lean [direction] 🔴 HIGH RISK** | max $25 | O/U bet suppressed; GVI lean shown per §3.12; ML still eligible |
...
| Confidence < 50 (O/U) | **Lean [direction] 🔴 HIGH RISK** | max $25 | Outside target zone — no standard O/U bet; lean per §3.12 |
| Confidence ≥ 65 O/U (P9_BAN) | **Lean [direction] 🟡 MODERATE RISK** | $25–37 | Capped; lean shown; ML at 65–69 still eligible at $75 |
| Corrected projected total gap 1.0–1.9 runs vs line | **Lean [direction] 🟡 MODERATE RISK** | $25 lean | Insufficient gap for standard bet; direction shown |
| Corrected projected total gap < 1.0 runs vs line | **Lean [direction] 🔴 HIGH RISK** | max $25 | Near-zero gap; tracking lean only |
| Any Under gate 0 or A–E fails | **Lean [direction] per risk level** | per §3.12 | ML still eligible if conf 50–64; Under bet suppressed with lean shown |
...
- If `|Projected Total − O/U Line| < 2.0 runs` → O/U bet suppressed; output **Lean [direction] (🟡 MODERATE RISK — $25 lean: gap [x] < 2.0)** per §3.12; direction always shown
- **Exception:** When P10_MATCH is active (corrected total ≤ 6.5), gap threshold = **≥ 1.5 runs** (see §3.9 P10 Gap Exception)
- If gap ≥ 2.0 runs (or ≥ 1.5 for P10) AND DH G2 flag active AND direction = UNDER → output **Lean UNDER (🟡 MODERATE RISK — $25: DH G2 no UNDER bet)** per §3.12
...
```json
{
  ...
  "ou_over_pct": 61,
  "ou_risk_level": "STANDARD",
  "ou_bet_eligible": true,
  "ou_bet_size": "$75",
  "confidence_score": 72,
...
> **Critical:** Return only valid JSON. No markdown fences, no preamble. Use `"Data Notice: [field] not found"` for unavailable fields. The `ml_recommendation` field must output either `"ML $75 — conf [x] in zone"` or `"Pass — conf outside 50–64"` — never output a banned/suppressed message (P6_BAN was lifted in v2.8). The `ou_prediction` field must ALWAYS be "OVER" or "UNDER" — never "PASS" (see §3.12). Use `ou_risk_level` to communicate bet eligibility: "STANDARD" (🟢 bet eligible), "MODERATE RISK" (🟡 $25–37 lean), "HIGH RISK" (🔴 max $25 lean), "EXTREME RISK" (⚫ tracking only, no bet). `ou_bet_eligible`: true when standard bet applies, false when lean-only. `ou_bet_size`: actual recommended stake ("$75", "$50", "$25", "$0").
...
| Version | Changes |
|---------|---------|
| v3.16 | **BC/BW joint analysis — 1239-game joint ML+O/U outcome study (2026-07-28), new signal family layered on top of v3.15's combo/fade system.** **(1) Both-Correct (BC) signals:** 7 new codes (BC1–BC7) added to `combo_hits[]` as a third code family alongside MC/OC, representing flag combinations where ML AND O/U have historically landed correctly together at well above the 26.5% baseline. BC2 (WPA+UNDER+Line 8-9, n=21) and BC4 (Home pred+WPA+UNDER, n=19) carry **zero recorded both-wrong games** — confirmed as the framework's single most trustworthy dual-stake signal. BC7 (Golden Condition+Conf 50-55, n=83★) is the largest sample. **(2) Both-Wrong (BW) signals:** 7 new codes (BW1–BW7) added to a **new dedicated `bw_signals[]` array and DB column** (idempotent migration, following the same pattern as `combo_hits`/`fade_signals`/`team_signals`/`team_combo_signals`) — these are hard double-PASS overrides, not fades: the joint failure mode isn't clean enough to bet the reverse side confidently on both markets, so when a BW code fires, both `ml_recommendation` and the O/U bet are set to "PASS BOTH MARKETS" regardless of what else fired, checked before any other bet-sizing logic. BW1 (Surging Away SP+Golden Condition, 60.0%) is the worst combo found; BW7 (OU-A+OU-B+Surging Away SP, n=20★) is the largest sample. **(3) Cross-cutting finding:** `away_surge` (surging away starter) appears in 6 of the top 7 BW combos, confirming — from a third independent analytical angle after the FD/FDOU fade families — that it is the framework's most damaging recurring blind spot. **(4) Reference material:** `public/combos.html` gained 4 new tabs (BC/BW trend, Both-Correct combos, Both-Wrong combos, Single-flag BC/BW); `public/app.js` gained BC chip rendering (green, "Bet Both Markets") and a new BW warning group (red, "PASS BOTH MARKETS," rendered first/most prominent since it's a hard override). |
| v3.15 | **Combo/fade signal refresh — 1363 games through Jul 27 (2026-07-28), new Reverse O/U signal family.** MC1–10, OC1–10, and FD1–10 re-derived from the 1363-game dataset (+134 games since v3.14/1229g). **(1) MC1–10 refreshed:** the WP-Override family (WPA+WPB combined) anchors 7 of 10 top ML combos; MC2 (Home pred+WPA+UNDER) and MC6 (WPA+UNDER+Line 8-9) are now flagged as having **zero recorded both-wrong games** — the two highest-trust dual plays in the system. MC4 (Home Fortress+WPA+Line 8-9) carries a new caution: collapses to 50% both-wrong when away_surge co-fires. MC10 (Home pred+WP gap≥20%+WPA, n=25★) is the largest-sample top-10 ML combo. **(2) OC1–10 refreshed:** away_surge now anchors 4 of the top 10 O/U combos (OC2, OC3, OC4, OC10) — confirmed as a broken ML signal but a genuinely useful O/U signal in the right context (GVI≥65, MCF, or Line 8-9+UNDER); OC9 (Conf 50-55+Dome+RCF, n=31★) remains the largest-sample dual-market O/U combo. **(3) FD1–10 refreshed:** away_surge still anchors all 10 ML fades; FD1 (HFCF+WPA+Away surge, 83.3%) is the strongest fade this update, FD9 (OU-B+Line 8-9+Away surge, n=40★) remains the largest-sample fade. Cross-referenced against the both-correct/both-wrong joint analysis: FD2 and FD4 are also top-5 both-wrong combos — flagged for a full PASS over a confident reverse. **(4) NEW — Reverse O/U signal family (FDOU1–10):** a distinct signal type that flips the model's O/U call itself (not the ML pick), surfaced via the same `fade_signals[]` array using an `FDOU`-prefixed code (no DB schema change — reuses existing storage/export plumbing). FDOU1–3 cluster around Conf 55-65 + away_surge (FDOU3, n=19★, is the largest-sample O/U fade found). FDOU5/FDOU6 (GVI<35+TMF) are a rare case where the model's **UNDER** call fails and OVER is the correct reverse — the opposite direction from every other O/U fade in this update and worth tracking against the §3.13 base-rate OVER-bias correction as a documented exception, not a contradiction. FDOU10 carries a noted data-quality caveat (source analysis lists two conditions — "surging SP" and "away surge RED" — that may reflect the same underlying signal) and should be treated with reduced confidence pending re-verification. **(5) Season-to-date context:** overall season ML 54.3% / OU 47.5%; since the All-Star break, ML has improved to 58.5% while OU has stayed weak at 46.9% — continue leaning on ML and the reverse-signal families over standalone O/U calls until OU recovers. `public/combos.html` updated in parallel with a new "Reverse O/U signals" tab. |
| v3.14 | **Full flag audit — 1229-game structural recalibration (2026-07-18).** A dedicated audit of every named flag in the framework against the full 1229-game dataset. **(1) Structural finding (new §3.13, highest priority):** the model has predicted OVER on 70.1% of all games all season (n=1124) while actual outcomes split 49.6%/50.4% — a season-long +20.5pp structural bias, not a recent drift. `OVER_RATE_GATE` (§3.6) is reclassified from rolling-window emergency patch to **permanent structural correction** active every game, and the GVI dead-zone / no-signal default lean shifts from month-dependent to a base UNDER-leaning presumption pending a named catalyst. **(2) Overstated flags corrected:** HFCF ML recalibrated 85.7%→59.5% (n=126, "best predictor" claim dropped, still Tier 1); RCF+Slumping OVER demoted from a strong 65% signal to informational-only (49.6% at the same n=113 — a genuine decay, not dilution); GVI≥90 hard pass softened to caution pending re-test (GVI≥90 alone now 57.1% ML at n=63, an inversion of the original 40.9%/n=22 finding — the exact triple condition GVI≥90+OVER+line≥9 has not been re-tested); GVI≥65 May+ catalyst gate stat updated 44.9%→53.4% (n=423, recovered but gate retained pending isolated re-test). **(3) Understated flag promoted:** WP-Override B raised from "weak secondary signal only" (~47%) to a **primary ML driver on par with WPA** — 60.8% ML at n=102, now the 3rd-strongest individual flag in the framework; award increased from +5% to +10% WP. **(4) Flagged for re-test/investigation, not yet changed:** OU-B (environmental signal, the largest-sample flag in the system at n=620) is landing at 50.2% OU — a coin flip — and is suspected as a major contributor to the OVER bias, pending isolated re-test after the base-rate correction; Dome games (n=284) are landing at 44.0% O/U, a possible sign/direction error under investigation. **(5) Implementation bug fixed:** R14's away-ace WP-flip reconciliation used a flat ±10-point adjustment that could fail to cross the predicted winner to away when the pre-R14 home-favoring gap was ≥20 points, causing grading to score some correct "bet away" calls as home picks (recorded 47.8% ML vs the underlying 53.5% away win rate); fixed to force a real ≥6-point crossover and to also check `active_overrides` when detecting R14. **(6) Well-calibrated, confirmed unchanged:** WPA (64.1% vs 63.0% documented, best-calibrated flag in the system), PDCF+conf<45 (both-markets-PASS still justified), Conf 50–55 O/U zone (54.1% vs 56.6% documented), GVI 35–65 dead zone (46.5% vs "no edge," n=510, confirmed). |
| v3.13 | **1229-game empirical refresh + corrected reverse signal rule (2026-07-18).** Key fix: fade/reverse signals were hardcoded as "Fade ML to AWAY $75" — but 6 of 10 combos are genuinely mixed directional (model named home in some games, away in others). The correct instruction is now: **REVERSE whatever team the model's betting_recommendation actually names for that specific game.** (1) **FD1–FD10 corrected** — each code now carries "MIXED" vs "one-sided" label; action reads "REVERSE model's named team" not "Fade ML to AWAY." FD9 (OU-B + Line 8-9 + Away surge, n=40★) is largest-sample fade. FD1 (HFCF + OU-A + Away surge, 81.2%) is highest-rate fade. (2) **MC1–MC10 refreshed (1229g)** — WPA now anchors 7 of 10 top ML combos (WPB previously dominated). MC5 (Home pred + WP gap≥20% + WPA, n=24) is new largest-n top combo. MC9 (WPA + UNDER + Line 8-9, 85%/65% OU) is best dual-market. (3) **OC1–OC10 refreshed (1229g)** — OC2 carries active fade warning (O/U 75% reliable but ML 25% = flip via FD6). OC7 (Conf 50-55 + Dome + RCF, n=31★) largest dual-market sample. OC9 (Line 8-9 + Conf 55-65 + RCF, n=35★) largest O/U-only. (4) **Team 2-flag combos refreshed (1229g)** — ATH added (gvi65+c50, 100%/n=8, best dual-market new entry); KC changed to ob_fired+under (100%/n=8); STL changed to rcf+c50 (90%/55.6%); TB updated to 90.9%/n=11. (5) **Analyzer model upgraded** — image analysis and text/JSON parse endpoints now default to claude-sonnet-5. |
| v3.12 | **1108-game refresh + O/U diagnostic fix (2026-07-05).** Key finding: the model had a +40pp OVER-prediction bias over 40 graded games (predicted OVER 82.5%, actual 42.5%) — a 6-week drift. Three-pronged fix applied: (1) **OVER_RATE_GATE emergency injection** — when rolling 5-day actual-OVER rate <45%, a hard-to-miss caution block is now added to the user-message context before game data, explicitly suppressing GVI-only OVER calls and defaulting O/U lean to UNDER. (2) **MC1–MC10 refreshed** — MC3 now HFCF+UNDER+MCF (91.7%, NEW); MC6 Home Fortress+WPB+Conf50-55 grows to n=27 (most reliable by sample size); MC8 adds Home pred+WP gap≥20%+WPA (85.7% n=21). (3) **OC1–OC10 refreshed** — OC8 (TMS home higher+Conf50-55+RCF) grows to n=52★★ (largest sample in entire playbook); OC5 adds RED mismatch+Conf50-55+GVI<35 (76.9%, NEW). (4) **FD1–FD10 refreshed** — FD10 (OU-B+Line8-9+Away surge, n=37★) is the new largest-sample fade; FD1 is now HFCF+OU-A+Away surge (78.6%). (5) **Team 2-flag combos refreshed** — KC/MIA/TB now hold perfect 100% ML records; STL added (oua+home_ft 90.9%); wp_lo (WP gap<10%) added as new helper. (6) **Team signal scope fix** — server now strips team_signals/team_combo_signals entries for non-playing teams (e.g. ATL code appearing in STL@CHC game). (7) **O/U diagnostic tab** added to /combos with bias visualization and 6-week drift table. |
| v3.11 | **1018-game empirical refresh + Team 2-Flag Combo system (2026-06-28).** All 30 MC/OC/FD combo codes re-derived again from a larger 1018-game dataset, plus a new higher-precision team-correlation layer. (1) **MC1–MC10 refreshed** — WP-Override B has overtaken WPA as the dominant ML backbone, appearing in 4 of the top 6 combos; new top combo is Home pred + GVI<35 + Dome (93.3% ML, n=15). (2) **OC1–OC10 refreshed** — new n=47★ "TMS home higher + Conf 50–55 + RCF" combo (74.5% OU, 67.9% ML) is the most statistically trustworthy recurring O/U bet; RCF appears in 5 of 10, Conf 50–55 in 8 of 10. (3) **FD1–FD10 refreshed** — all 10 fade signals still share a confirmed surging away SP; top fade is WP gap≥20% + OU-B + Away surge (81.2%, n=16); FD5 (RED mismatch>1.5 + OU-A/B + Line 8–9 + Away surge, n=31★) is the largest-sample fade. (4) **Team 2-Flag Combo Table (NEW)** — 10 teams mapped to their strongest 2-flag pairing by ML win rate, min n=8 (e.g. Kansas City Royals + WP-Override B + Home Fortress = 100% ML n=11, 90.0% OU n=10 — best dual-market team signal in the dataset; LA Angels' only reliable combo is Home pred + UNDER, 88.9% ML n=9). Layered above (does not replace) the existing 30-team single-flag table. Surfaced via new `team_combo_signals[]` JSON field (format `"ABBR:flag1+flag2"`). (5) **`/combos` reference page** — added a "Team 2-flag combos" tab and a "Results update" diagnostics tab (5-period trend + Jun 22–27 day-by-day breakdown showing an O/U collapse to 30.6% staked while ML held at 55.1% — flagged for OVER_RATE_GATE verification). (6) **DB persistence** — added `team_combo_signals` column with idempotent startup migration, consistent with the existing `combo_hits`/`fade_signals`/`team_signals` pattern. |
| v3.10 | **931-game empirical refresh + Team Signal system (2026-06-21).** All 30 MC/OC/FD combo codes re-derived from a larger 931-game dataset, plus a new team-specific correlation table. (1) **MC1–MC10 refreshed** — new top combo is UNDER + Conf 50–55 + TMF (93.3% ML, n=15; 78.6% O/U simultaneously). (2) **OC1–OC10 refreshed** — OC9 (OU-B + Conf 50–55 + TMF) is the new large-n anchor (74.3% OU, n=35★, 71.1% ML). (3) **FD1–FD10 refreshed** — all 10 fade signals now share a confirmed surging away SP (away_red<−1.0) as the common ingredient, consistent with R14_AWAY_ACE_HARD; FD5 (RED mismatch>1.5 + OU-B + Away surge, n=41★) is the largest-sample, highest-confidence fade (75.6%). (4) **Team Signal Table (NEW)** — 30 teams mapped to their single strongest empirical ML-flag correlation, min n=8 (e.g. Texas Rangers + Golden Condition = 91.7% ML n=12; Miami Marlins + Home pred = 84.4% ML n=32, largest team sample; LA Angels weakest, no flag breaks 60%). Surfaced via new `team_signals[]` JSON field (format `"ABBR:flagname"`) and rendered as purple chips alongside combo/fade indicators. (5) **`/combos` reference page** — added "Top 10 teams" and "All 30 teams" tabs mirroring the new team-correlation data. (6) **DB persistence** — added `team_signals` column with idempotent startup migration; fixed a pre-existing gap where `combo_hits`/`fade_signals` were silently dropped on Railway cold-start reseed and `/api/import`. |
| v3.9 | **Combo & Fade Signal Indicator System (2026-05-31).** 30 named signal codes evaluated post-analysis and surfaced as colored UI chips. (1) **MC1–MC10 (ML Combos)** — 10 multi-condition ML combo codes; MC2 is the Crown Jewel (Home Fortress + conf 50–55 + TMF = 93% ML n=14, 85% O/U simultaneously). (2) **OC1–OC10 (O/U Combos)** — 10 multi-condition O/U combo codes; OC6 (TMF + Home Fortress) is the main recurring anchor (71% OU, n=35). (3) **FD1–FD10 (Fade Signals)** — 10 reversal/override codes replacing the prior R-prefix naming to avoid conflict with framework rule flags R1–R16; FD7 (OU-B + Away surge + model backs home) is highest-n fade (69%, n=45). (4) **UI chip rendering** — Fade signals shown first in red as warnings, ML combos in green, O/U combos in blue; block hidden when both arrays empty. (5) **JSON schema** — New fields: `combo_hits[]` and `fade_signals[]`. PREDICT_SYSTEM bumped to v3.9 with full COMBO SIGNAL EVALUATION section. |
| v3.8 | **534-game deep analysis — 8 rule adjustments (2026-05-19).** (1) **HFCF reclassified** — from −20 caution to ML CONFIRMATION Tier 1 $75 (85.7% ML, n=14); O/U still −20 (38.5%); single biggest accuracy lift available. (2) **PDCF blocks O/U (conf<45)** — PDCF + conf<45 = both ML and O/U PASS; OU 32.6% (n=43), ML 44.4% — both markets fail simultaneously; previously only ML was blocked. (3) **Conf<50 ML Gate** — conf<50 + WP gap<20% → ML PASS; 76% of May17+ ML losses had conf<50; 43.6% ML without WP quality advantage. (4) **GVI≥80 WP Reduction** — GVI≥80 → PVS-driven WP bonuses halved; GVI≥90+OVER+line≥9 = hard pass (40.9% ML, 45.0% OU, n=22). (5) **R14 AWAY_ACE_HARD** — upgraded from soft −10% WP to: (a) ML flip to away unconditionally (≥5 starts); (b) O/U SKIP all directions (away surge + UNDER = 35%, + OVER = 51.7% — both unreliable). (6) **OUA+OUB+OVER elevated to Tier 1** — 57.8% OU, n=109 (largest reliable O/U sample; outperforms Golden Condition at n=76). (7) **Home SP RED<−1.0 → O/U UNDER lean (Condition 5 NEW)** — 64.0% OU in May, n=25; converts home surge from pure ML-penalty to dual-use signal. (8) **R8 MCF refined** — MCF+Slump+WP≥10% → ML $75; MCF+Slump+WP<10% → $37.50; MCF+no-Slump → ML ban (43.1%). |
| v3.7 | **437-game OVER environment collapse correction (2026-05-12). 6 targeted fixes for May OVER misfires.** (1) **OVER_RATE_GATE (R15 NEW)** — rolling 5-day actual OVER rate gate: <44%=−15 conf + RED mismatch >2.0 required; <42%=GVI-only OVER blocked; <38%=2 primary signals required. Root cause: May 8–10 rate=36%, model called OVER 84% — 24/26 BW games had OVER→UNDER misfire. (2) **May+ GVI OVER Catalyst Gate (OU-E updated)** — GVI≥65 OVER in May+ requires Slumping SP RED>+1.5 OR Wind OUT>12mph+temp>65°F OR both wRC+>105 OR line 9–10. GVI from PVS alone = PASS. Evidence: GVI≥65 OVER dropped 56.6%→44.9% in May. (3) **R7 updated** — GVI65+ OVER viable in April (58.9%) but inverted in May without catalyst (44.9%); added catalyst gate to R7 flag output. (4) **R8 MCF flip (NEW)** — MCF active + OVER direction → flip to UNDER (market pricing suppression). MCF+UNDER=63.6% May OU; MCF+OVER remaining=41.8%. (5) **R16 NEW** — May conf 50–55 + OVER requires 2 named signals (was 1); conf 50–55 + OVER + 1 signal only = 33% in May (destructive). (6) **May bias dynamic (§3.9 updated)** — base +2.0 runs; when rolling 7-day avg actual <8.5, reduce to +1.5 runs (model was projecting 0.6 runs too high in May 8–10). 5 new confidence deductions: ORC(−15), ORS(−25), ORHB(−30), OGCF(−20), MCOW(−20). UNDER prediction in May improved to 63.6% (from 47.7%) — system should actively shift toward UNDER in compressed scoring environments. |
| v3.6 | **Never-Pass O/U Direction Policy (2026-05-05).** §3.12 added: model must always output OVER or UNDER — never bare "PASS." Ban rules and dead zones become risk level escalators (⚫ EXTREME RISK / 🔴 HIGH RISK / 🟡 MODERATE RISK / 🟢 STANDARD) rather than direction eliminators. Step 1 lean hierarchy: GVI≥65→OVER, GVI<35→UNDER, GVI 35–65→subsidiary signals (Slumping SP, wind, park, temperature; May+ default UNDER, April default OVER). Step 2 risk assignment based on which ban rules fired. Step 3 output format always includes direction + risk annotation. Affected rules: OU-E dead zone (GVI 35–65) → HIGH RISK lean; OU-F April no-signal → EXTREME RISK lean; BSS Line Cap → MODERATE RISK lean; R1 → EXTREME RISK lean; R7 GVI<35 UNDER → EXTREME RISK (bet still banned); R12 dead zone → HIGH RISK lean. JSON schema: `ou_prediction` always OVER/UNDER; new fields `ou_risk_level`, `ou_bet_eligible`, `ou_bet_size`. Empirical basis: 50 graded PASS games = 46% OVER / 54% UNDER — useful directional signal suppressed by prior PASS policy. |
...
*MLB Game Predictor — CLAUDE.md v3.16*
