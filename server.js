...
**§3.11 ACTION RULES v3.8 — 534-game empirical revision. R8 refined, R14 upgraded, new Condition 5 OU-A, 8 adjustments:**
...
R14 (UPGRADED v3.8 — AWAY_ACE_HARD): Away SP RED<−1.0 AND >=5 confirmed starts → TWO hard rules: (1) ML: route to AWAY team unconditionally — flip direction if model was backing home (overrides WPB/Home Fortress/TMS home). (2) O/U: SKIP all O/U bets on this game — away surge + UNDER = 35.0% OU (consistent loss), away surge + OVER = 51.7% OU (near breakeven) — both directions are unreliable. Total is unpredictable when away ace surging. Use R14 for ML direction only. Flag: R14_AWAY_ACE_HARD — "Away SP RED<−1.0 ≥5 starts: ML=AWAY unconditional + O/U=SKIP both directions (35% UNDER / 51.7% OVER — both fail)."
